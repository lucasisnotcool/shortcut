...
- Press left and right Option together to capture the active app window and check its visible question. The menu-bar circle spins while Claude works, then shows the answer: one option (`1`–`8` or `A`–`H`) in a ring; several options for a multiple-response question (for example `1 3 4`) in a widened capsule; `T` or `F` for true/false; `!` when Claude declines; `×` on error. Claude judges every option against the question as worded (so "Which are NOT…" is handled per option, and a multiple-response question can have a single answer), bases each verdict on the course files, then its own knowledge, then web search, and ignores anything already ticked or highlighted on screen. The chat and the badge's menu list each option with ✓ or ✗ and a one-line reason.
